/**
 * Cutelittleagent - Advanced AI Assistant
 * v2.8.0 (Updated: AI Pipe integration + bug fixes)
 * Author: Ashish Kumar Yadav (Original) & Assistant (fixes)
 */
class Cutelittleagent {
    constructor() {
        this.version = '2.8.0';
    ...
    async init() {
        try {
            ...
            console.log(`Cutelittleagent v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize Cutelittleagent:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'Cutelittleagent', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to Cutelittleagent! How can I assist you?');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentFlow = new Cutelittleagent();
});
